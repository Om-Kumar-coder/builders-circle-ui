...
import { Users, Zap, CheckCircle, XCircle, RotateCcw, AlertTriangle, Clock, ChevronLeft } from 'lucide-react';
...
export default function IntakePage() {
  ...
  const fetchIntake = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiClient.getGatekeeperIntake({
  ...
  const handleScan = async (triageId: string) => {
    setScanning(triageId);
    try {
      await apiClient.scanIntakeWithVeronica(triageId);
      await fetchIntake();
    } finally {
      setScanning(null);
    }
    ...
    <MainLayout>
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-white">User Intake Queue</h1>
      ...
      {selectedReview && (
        <GatekeeperActionModal
          ...
          onSuccess={() => { setSelectedReview(null); fetchIntake(); }}
        />
      )}
    </MainLayout>
  );
